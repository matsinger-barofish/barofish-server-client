...
public class UserController {
    ...
    public ResponseEntity<CustomResponse<Jwt>> joinSnsUser(@RequestPart(value = "data") SnsJoinReq data) {
        ...
        try {
            if (data.getLoginType() == null)
                return res.throwError("로그인 타입을 입력해주세요.", "INPUT_CHECK_REQUIRED");
            if (data.getLoginId() == null)
                return res.throwError("로그인 아이디를 입력해주세요.", "INPUT_CHECK_REQUIRED");
            ...
            if (userAuth.isPresent()) {
                ...
                if (user.getState().equals(UserState.BANNED))
                    return res.throwError("정지된 사용자입니다.", "NOT_ALLOWED");
                ...
                if (userService.checkExistWithPhone(phone))
                    return res.throwError("회원가입된 이력이 있습니다.", "NOT_ALLOWED");
                if (data.profileImage != null) {
                    profileImage = s3.upload(s3.extractBase64FromImageUrl(data.profileImage),
                ...
                UserInfo userInfo = UserInfo.builder().userId(user.getId())
                        ...
                        .email(data.email != null ? data.email : "").profileImage(
                                profileImage)
                        .name(data.name != null ? data.name : "").grade(grade)
                        .phone(data.phone != null ? data.phone : null).point(point).isAgreeMarketing(false).build();
                userService.addUserInfo(userInfo);
                UserAuth newUserAuth = userService
                        .addUserAuth(UserAuth.builder().userId(user.getId()).loginId(data.getLoginId()).loginType(
            ...
            if (!user.getState().equals(UserState.ACTIVE)) {
                if (user.getState().equals(UserState.BANNED))
                    return res.throwError("정지된 유저입니다..", "NOT_ALLOWED");
                if (user.getState().equals(UserState.DELETED))
                    return res.throwError("삭제된 유저입니다.", "NOT_ALLOWED");
            }
    ...
    public ResponseEntity<CustomResponse<Boolean>> joinUser(@RequestPart(value = "data") UserJoinReq data,
        ...
        try {
            ...
            if (!Pattern.matches(re.email, email))
                return res.throwError("이메일 형식을 확인해주세요.", "INPUT_CHECK_REQUIRED");
            String nickname = util.validateString(data.getNickname(), 50L, "닉네임");
            if (userService.checkExistWithNickname(nickname))
                return res.throwError("중복된 닉네임입니다.", "NOT_ALLOWED");
            String name = util.validateString(data.getName(), 20L, "이름");
            if (data.postalCode == null)
                return res.throwError("우편 번호를 입력해주세요.", "INPUT_CHECK_REQUIRED");
            ...
            String imageUrl = profileImage != null && !profileImage.isEmpty() ? s3.upload(profileImage,
                    new ArrayList<>(Arrays.asList("user", String.valueOf(user.getId()))))
                    : String.join("/",
            ...
            SiteInformation siteInfo = siteInfoService.selectSiteInfo("INT_JOIN_POINT");
            Integer point = Integer.parseInt(siteInfo.getContent());
            UserInfo userInfo = UserInfo.builder().phone(phone).nickname(nickname).email(email).name(name)
                    .profileImage(imageUrl).grade(
                            grade)
                    .point(point).isAgreeMarketing(isAgreeMarketing).build();
            DeliverPlace deliverPlace = DeliverPlace.builder().name(name).receiverName(name).tel(phone).address(address)
                    .addressDetail(
                            addressDetail)
                    .deliverMessage("").postalCode(data.postalCode).isDefault(true).build();
    ...
    public ResponseEntity<CustomResponse<UserInfoDto>> updateUser(@RequestHeader("Authorization") Optional<String> auth,
        ...
        try {
            ...
            if (profileImage != null) {
                String imageUrl = s3.upload(profileImage,
            ...
            if (data.getNewPassword() != null) {
                if (data.getOldPassword() == null)
                    return res.throwError("변경 전 비밀번호를 입력해주세요.", "INPUT_CHECK_REQUIRED");
            ...
            if (data.getPhone() != null) {
                if (data.getVerificationId() == null)
                    return res.throwError("인증 먼저 진행해주세요.", "INPUT_CHECK_REQUIRED");
                if (!Pattern.matches(re.phone, data.phone))
    ...
    @GetMapping("/mypage")
    public ResponseEntity<CustomResponse<UserInfoDto>> selectUserSelfInfo(
        ...
        try {
            ...
            UserInfoDto userInfoDto = UserInfoDto.builder().userId(userId).profileImage(userInfo.getProfileImage())
                    .grade(userInfo.getGrade()).email(
                            userInfo.getEmail())
                    .name(userInfo.getName()).nickname(userInfo.getNickname()).phone(
                            userInfo.getPhone())
                    .isAgreeMarketing(userInfo.getIsAgreeMarketing()).reviewCount(
                            reviewService.countAllReviewByUserId(userId))
                    .notificationCount(notificationService.countAllNotificationByUserId(
                            userId))
                    .deliverPlaces(deliverPlaces).point(userInfo.getPoint()).build();
    ...
    public ResponseEntity<CustomResponse<UserInfoDto>> selectUserList(
    ...
    public ResponseEntity<CustomResponse<Page<UserInfoDto>>> selectUserList(
            @RequestHeader("Authorization") Optional<String> auth,
        ...
        try {
            Specification<UserInfo> spec = (root, query, builder) -> {
                ...
                if (nickname != null)
                    predicates.add(builder.like(root.get("nickname"), "%" + nickname + "%"));
                if (email != null)
                ...
                if (joinAtS != null)
                    predicates.add(builder.greaterThan(root.get("user").get("joinAt"), joinAtS));
                if (joinAtE != null)
                    predicates.add(builder.lessThan(root.get("user").get("joinAt"), joinAtE));
    ...
    public ResponseEntity<CustomResponse<Boolean>> updateUserState(
            @RequestHeader(value = "Authorization") Optional<String> auth,
        ...
        try {
            if (data.state == null)
                return res.throwError("상태를 입력해주세요.", "INPUT_CHECK_REQUIRED");
    ...
    public ResponseEntity<CustomResponse<String>> whoami(
            @RequestHeader(value = "Authorization") Optional<String> auth) {
    ...
    public ResponseEntity<CustomResponse<Boolean>> withdrawUser(
            @RequestHeader(value = "Authorization") Optional<String> auth) {
        CustomResponse<Boolean> res = new CustomResponse<>();
        Optional<TokenInfo> tokenInfo = jwt.validateAndGetTokenInfo(Set.of(TokenAuthType.USER), auth);
        if (tokenInfo == null)
        ...
        try {
            ...
            // 장바구니 제거
            List<Integer> basketIds = basketService.selectBasketList(userId).stream().map(BasketProductDto::getId)
                    .toList();
    ...
    public ResponseEntity<CustomResponse<Boolean>> updateFcm(
            @RequestHeader(value = "Authorization") Optional<String> auth,
        ...
        try {
            ...
            if (data.fcmToken == null)
                return res.throwError("토큰을 입력해주세요.", "INPUT_CHECK_REQUIRED");
    ...
    public ResponseEntity<CustomResponse<List<PaymentMethodDto>>> selectPaymentMethodList(
            @RequestHeader(value = "Authorization") Optional<String> auth,
        ...
        try {
            if (tokenInfo.get().getType().equals(TokenAuthType.USER)) {
                ...
                if (userId == null)
                    return res.throwError("유저 아이디를 입력해주세요.", "INPUT_CHECK_REQUIRED");
            }
            User user = userService.selectUser(userId);
    ...
    public ResponseEntity<CustomResponse<PaymentMethodDto>> selectPaymentMethod(
            @RequestHeader(value = "Authorization") Optional<String> auth,
            @PathVariable("id") Integer id) {
    ...
    public ResponseEntity<CustomResponse<PaymentMethodDto>> addPaymentMethod(
            @RequestHeader(value = "Authorization") Optional<String> auth,
        ...
        try {
            ...
            if (data.cardNo == null)
                return res.throwError("카드번호를 입력해주세요.", "INPUT_CHECK_REQUIRED");
            ...
            PaymentMethod paymentMethod = PaymentMethod.builder().name(name).cardNo(aes256.encrypt(cardNo))
                    .userId(tokenInfo.get().getId()).expiryAt(
                            data.expiryAt)
                    .birth(birth).passwordTwoDigit(password2Digit).build();
            PaymentService.CheckValidCardRes validCardRes = paymentService.checkValidCard(paymentMethod);
            if (validCardRes == null)
                return res.throwError("유효하지 않은 카드입니다.", "INPUT_CHECK_REQUIRED");
    ...
    public ResponseEntity<CustomResponse<Boolean>> deletePaymentMethod(
            @RequestHeader(value = "Authorization") Optional<String> auth,
            @PathVariable("id") Integer id) {
        CustomResponse<Boolean> res = new CustomResponse<>();
        Optional<TokenInfo> tokenInfo = jwt.validateAndGetTokenInfo(Set.of(TokenAuthType.USER), auth);
