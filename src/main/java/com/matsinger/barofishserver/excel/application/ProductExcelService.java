...
public class ProductExcelService {
    ...
    public ProductExcelData convertRow2ProductData(Row row) {
        ...
        if (store == null || store.isEmpty()) throw new IllegalArgumentException(String.format("%s 는 없는 입점사 ID 입니다.", partnerId));
        // 3 상품명
        String productName = row.getCell(3).getStringCellValue();
        if (productName == null) throw new IllegalArgumentException("상품명이 비었습니다. 확인해주세요.");
        Optional<Product>
                product =
        ...
        if (parentCategoryName == null) throw new IllegalArgumentException("1차 카테고리가 비었습니다. 확인해주세요.");
        ...
        if (parentCategory == null || parentCategory.isEmpty())
            throw new IllegalArgumentException(String.format("%s는 존재하지 않는 1차 카테고리입니다.", parentCategoryName));
        // 2 2차 카테고리
        String categoryName = row.getCell(2) != null ? row.getCell(2).getStringCellValue() : null;
        if (categoryName == null) throw new IllegalArgumentException("2차 카테고리가 비었습니다. 확인해주세요.");
        Optional<Category>
                category =
                categoryQueryService.findOptionalCategoryWithName(categoryName, parentCategory.get().getId());
        if (category == null || category.isEmpty())
            throw new IllegalArgumentException(String.format("%s는 존재하지 않는 2차 카테고리입니다.", categoryName));
        // 4 도착 예정일
        ...
        if (deliveryFeeCell == null) throw new IllegalArgumentException(String.format("'%s' 의 배송비를 확인해주세요. ", productName));
        ...
        if (deliverBoxPerAmountCell == null)
            throw new IllegalArgumentException(String.format("'%s' 의 택배 책정 수량을 확인해주세요. ", productName));
        ...
        if (optionNameCell == null) throw new IllegalArgumentException(String.format("'%s' 의 옵션명을 확인해주세요. ", productName));
        List<String> optionNameList = Arrays.stream(row.getCell(13).getStringCellValue().split("\n")).toList();
        // 11 매입가
        Cell purchasePriceCell = row.getCell(11);
        if (purchasePriceCell == null) throw new IllegalArgumentException(String.format("'%s' 의 매입가를 확인해주세요. ", productName));
        ...
        if (originPriceCell == null) throw new IllegalArgumentException(String.format("'%s' 의 옵션 정가를 확인해주세요. ", productName));
        List<Integer>
                originPriceList =
        ...
        if (discountPriceCell == null) throw new IllegalArgumentException(String.format("'%s' 의 옵션 할인가 확인해주세요. ", productName));
        List<Integer>
                discountPriceList =
        ...
        if (maxAvailableAmountCell == null) throw new IllegalArgumentException(String.format("'%s' 의 최대 주문 수량을 확인해주세요.", productName));
        List<Integer>
        ...
        if (amountCell == null) throw new IllegalArgumentException(String.format("'%s' 의 재고를 확인해주세요.", productName));
        List<Integer>
                amountList =
        ...
        if (new HashSet<>(List.of(purchasePriceList.size(),
                ...
                amountList.size())).size() != 1) throw new IllegalArgumentException("옵션 정보의 개수를 일치시켜 주세요.");
