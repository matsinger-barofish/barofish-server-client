    ...
    static class TestUserService {
        ...
        public void createUser() {
            for (int i = 1; i < 3; i++) {
                ...
                log.info("### findByLoginTypeAndLoginId() 메서드 실행 ###");
//                UserAuth findUserAuth = userAuthRepository.findByLoginTypeAndLoginId(LoginType.IDPW, "test" + i)
//                        .orElseThrow(() -> {
//                            throw new IllegalStateException(OrderErrorMessage.USER_AUTH_NOT_FOUND_EXCEPTION);
//                        });
                log.info("### findByLoginTypeAndLoginId() 메서드 끝 ###");
    ...
    static class TestOrderService {
        ...
        public void createTestOrders() {
            int seq = 1;
            List<OrderRequestDto.OrderProductInfoDto> products = new ArrayList<>();
            for (int i = 1; i < 3; i++) {
                List<OrderRequestDto.OrderProductOptionDto> options = new ArrayList<>();

                for (int j = 1; j < 3; j++) {
                    OrderRequestDto.OrderProductOptionDto optionDto = OrderRequestDto.OrderProductOptionDto.builder()
                            .optionId(seq)
                ...
                OrderRequestDto.OrderProductInfoDto productInfoDto = OrderRequestDto.OrderProductInfoDto.builder()
                        .productId(i)
                        .originPrice(5000 * i)
                        .discountRate(i * 0.1)
                        .amount(10 + i)
