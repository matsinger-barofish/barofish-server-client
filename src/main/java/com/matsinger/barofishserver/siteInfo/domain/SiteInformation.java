...
import com.matsinger.barofishserver.siteInfo.dto.SiteInfoDto;
import com.matsinger.barofishserver.siteInfo.dto.TitleContentReq;
...
public class SiteInformation {
    ...
    public SiteInfoDto convert2Dto() {
        ...
        if (this.id.startsWith("TC")) {
            List<TitleContentReq> tcs = new ArrayList<>();
            JSONArray jsonArray = new JSONArray(this.content);
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject obj = jsonArray.getJSONObject(i);
                TitleContentReq
                        tc =
                        TitleContentReq.builder().title(obj.getString("title")).content(obj.getString("content")).build();
                tcs.add(tc);
            }
